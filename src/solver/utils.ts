...
export const computeProducibleItems = (
  ...
  // Seed: raw materials available in this region
  ...
  // Seed: cycle items (seeds/plants) — the solver handles the loop
  ...
  while (changed) {
    ...
    for (const [itemId, recipes] of context.recipesByOutput) {
      ...
      for (const recipe of recipes) {
        ...
        const allInputsProducible = recipe.inputs.every((input) =>
